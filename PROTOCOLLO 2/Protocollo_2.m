...
% for i=2:6
% 
%     plot((i-1)*5000+emg_test(:,i));
%     hold on;
% end

emg_test_new=emg_test(1:233266,2:6);
...
label_test=[ones(16341,1);...
    ...
    zeros(77034-64774,1);...
    ones(89773-77034,1);...
    zeros(102440-89773,1);...
    ones(117008-102440,1);...
    zeros(129802-117008,1);...
    ones(145131-129802,1);...
    zeros(158721-145131,1);...
    ones(171239-158721,1);...
    zeros(183551-171239,1);...
    ones(195754-183551,1);...
    zeros(208558-195754,1);...
    ones(221858-208558,1);...
    zeros(233149-221858,1)];
% figure
% plot(emg_test_new(:,2));
% hold on;
...
emg_open_new=[emg_open(1:247328,2:6);emg_open(87248:112147,2:6)];

% % PLOT APERTURA RITOCCATA
% figure
% for i=1:5
% 
...
emg_close_new=emg_close(39000:end,2:6);

% % PLOT CHIUSURA RITOCCATA
% figure
% for i=1:5
% 
%     plot((i)*5000+emg_close_new(:,i));
%     hold on;
% end
emg_close_max_new=emg_close_max(17001:29000,2:end);
emg_open_max_new=emg_open_max(7201:19200,2:end);
% 
% % PLOT CHIUSURA MAX
% figure
% for i=1:5
...
% for i=1:5
% 
%     plot((i)*1000+emg_env_close(:,i));
%     hold on;
% end
% figure
% plot(emg_env_close(:,2));
th_close=65;
ind_cl=find(emg_env_close(:,2)>th_close);
% figure
% for i=1:5
% 
...
% for i=1:5
% 
%     plot((i)*1000+emg_env_test(:,i));
%     hold on;
% end
% figure
% plot(emg_env_test(:,2));
th_test=41;
ind_test=find(emg_env_test(:,2)>th_test);
% figure
% for i=1:5
% 
...
for i = 1:5
    ...
    % Ottieni la lunghezza del segnale
    signal_length = length(emg_op_cl(:,i));
    
    % Determina l'indice di metà segnale
    mid_index = floor(signal_length / 2);
    
    % Tempo per la prima metà del segnale
    time_first_half = (0:mid_index-1) / fsamp;
    
    % Tempo per la seconda metà del segnale
    time_second_half = (mid_index:signal_length-1) / fsamp;
    
    % Plot della prima metà in rosso
    plot(time_first_half, emg_op_cl(1:mid_index, i), 'r');
    hold on;
    
    % Plot della seconda metà in blu
    plot(time_second_half, emg_op_cl(mid_index+1:end, i), 'b');
    hold on;

    plot((0:length(emg_env(:,1))-1)/fsamp,emg_env(:,i), 'k')
    
    % Etichette e titolo
    xlabel('Tempo (s)');
    ylabel('Ampiezza (µV)');
    title(['Canale ', num2str(i)]);
    
    hold off;
end

% Titolo generale per la figura
sgtitle('Segnali grezzi senza riposo')

% Aggiungi la legenda
legend_labels = {'Apertura', 'Chiusura', 'Inviluppo'};
h = findobj(gcf, 'Type', 'axes');
legend(h(end), legend_labels);
...
for i = 1:5
    ...
    plot((0:length(emg_env(:,1))-1)/fsamp,emg_env(:,i), 'k', 'LineWidth', 1.25);
    
    % Aggiungi etichetta solo all'ultimo subplot
    if i == 5
        xlabel('Tempo (s)');
    else
        set(gca, 'XTickLabel', []);
    ...
    % Aggiungi etichette e titolo a destra
    ylabel('Ampiezza (µV)');
    text(1.05, 0.5, ['Canale ', num2str(i)], 'Units', 'normalized', 'HorizontalAlignment', 'left');
...
figure
for i = 1:5
    % Creazione del subplot con specifiche posizioni
    subplot('Position', [0.1, 0.1 + 0.18*(5-i), 0.7, 0.15])
    ...
    % Aggiungi etichette e titolo a destra, in verticale
    ylabel('Ampiezza (µV)');
    text(1.01, 0.5, ['Canale ', num2str(i)], 'Units', 'normalized', ...
...
iteration=round(length(emg_open_max_new)/overlap_int)-1;
for i=1:iteration
